...
class _SimulusOneInstance:
    ...
    Moved to module level (from nested inside _Simulus) so that pickle
    can locate the class by name, which is required for SMP mode where
    the _Simulus singleton is serialized and sent to child processes.
    """
...
class _Simulus(object):
    ...
    def __new__(cls):
        if not _Simulus.instance:
            _Simulus.instance = _SimulusOneInstance()
